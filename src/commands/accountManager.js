...
const showAccountMenu = (rl, client, callback) => {
  ...
  console.log("");
  console.log(colorful(colors.green, '     [1] Adicionar nova conta'));
...
const removeAccount = (rl, client, callback) => {
  ...
  if (!settings || !settings.accounts || settings.accounts.length === 0) {
    console.log(colorful(colors.red, '     [x] Nenhuma conta cadastrada'));
    setTimeout(callback, 2000);
    return;
  }
  ...
  rl.question('\n     [-] Número da conta para remover: ', (index) => {
    ...
    if (i >= 0 && i < settings.accounts.length) {
      ...
      console.log(colorful(colors.green, `     [✓] Conta ${removedAccount.username} removida com sucesso!`));
    } else {
      console.log(colorful(colors.red, '     [x] Índice inválido'));
    }
    setTimeout(callback, 2000);
  });
};
...
const selectAccount = (rl, client, callback) => {
  ...
  rl.question('\n     [-] Número da conta para usar: ', async (index) => {
    const i = parseInt(index) - 1;
    if (i >= 0 && i < settings.accounts.length) {
