...
const removeFriends = async (rl, client, settings, showMenu) => {
  ...
  console.log(colorful(colors.blue, '     [x] Removendo amizades...'));
  ...
  for (const friend of friends) {
    if (whitelistSet.has(friend.id)) {
      console.log(colorful(colors.yellow, `     [=] ${friend.user.username} está na whitelist, pulando...`));
      continue;
    }
  ...
  console.log(colorful(colors.green, `\n     [✓] Processo finalizado! ${removed} amizades removidas.`));
