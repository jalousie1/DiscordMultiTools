...
const downloadAttachment = async (url, folderPath, fileName) => {
  try {
    ...
    console.log(colorful(colors.red, `     [!] Erro ao baixar arquivo ${fileName}`));
  }
};
...
const exportChat = async (rl, client, callback) => {
  ...
  console.log(colorful(colors.blue, '     [x] Chat Exporter'));
  console.log(colorful(colors.green, '     [!] Você pode usar ID de usuário ou ID de canal'));
  ...
  rl.question('     [-] ID: ', async (id) => {
    try {
      ...
      console.log(colorful(colors.blue, `     [=] Exportando mensagens de ${chatName}...`));
      let messages = [];
      let lastId = null;
      let messagesFound = 0;
      ...
      while (true) {
        ...
        for (const msg of fetched.values()) {
          ...
          for (const attachment of msg.attachments.values()) {
            ...
            process.stdout.write(colorful(colors.yellow, 
              `     [+] Baixando anexo ${attachmentsFound}: ${attachment.name}`
            ));
        ...
        process.stdout.write(colorful(colors.green, 
          `     [+] Mensagens encontradas: ${messagesFound} | Anexos: ${attachmentsFound}`
        ));
      ...
      console.log(colorful(colors.green, 
        `\n     [✓] Export concluído!\n` +
        `     → ${messages.length} mensagens salvas em ${chatName}/messages.txt\n` +
        `     → ${attachmentsFound} anexos baixados em ${chatName}/attachments/`
      ));

    } catch (error) {
      console.log(colorful(colors.red, `     [x] Erro ao exportar chat: ${error}`));
    }
