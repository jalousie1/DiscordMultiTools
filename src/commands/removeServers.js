...
const removeServers = async (rl, client, settings, showMenu) => {
  ...
  console.log(colorful(colors.blue, '     [x] Removendo Servidores...'));
  ...
  for (const [, server] of servers) {
    if (whitelistSet.has(server.id)) {
      console.log(colorful(colors.yellow, `     [=] Servidor ${server.name} está na whitelist, pulando...`));
      continue;
    }
  ...
  console.log(colorful(colors.green, `\n     [✓] Processo finalizado! Saiu de ${removed} servidores.`));
